'''
Name: FixedTypeGraphWithRandomNetwork_TE_N_E_02
Description: For the parallel simulation of the random graph with fixed network 
and the time cost of the algorithm. 
...
try:
    ...
    import src.P2PAlgorithms as p2pA
    import src.P2PSTAlgorithms as p2pSTA
except:
...
#%% data Path
EXP_NAME='FixedTypeGraphWithRandomNetwork_TE_N_E_02'
...
if __name__=='__main__':
    ...
    vertexGroupNums=[9]*groupNum
    nodeGroupNums=np.linspace(10,200,groupNum,dtype=int)
    ...
    for i in range(0,groupNum):
        ...
        for j in range(0,batchNum):
            while True:
                task=p2pT.P2PTask(nodeGroupNums[i],pc=pc,channels=channels,topology='Waxman',\
            ...
            for k in range(0,batchSize):
                ...
                data[(i,j,k)]['AvgNodeNum']=np.mean(len(task.network_graph.nodes()))
...
def run_single_batch_simulation(task:p2pT.P2PTask, repeat:int=1, method:str='MGST'):
    totalResults=[]
    for t in task.network_task:
        start_time=tm.perf_counter()
        for _ in range(0,repeat):
            match method:
                case 'MGST': 
                    _,_=p2pA.find_MGST_paths(t.copy(),task.network_graph)
                case 'P2PGST': 
                    _,_,_=p2pA.find_P2P_GST_paths(t.copy(),task.network_graph)
                case 'ST-P2PGST-memShot': 
                    _,_=p2pSTA.memflow_bin_search(t.copy(),task.network_graph,\
                                                            memCostCoeff=1.0,memShot=True)
                case 'ST-P2PGST-memShot-Factor':
                    _,_=p2pSTA.memflow_bin_search(t.copy(),task.network_graph,\
                                                            memCostCoeff=1.0,memShot=1.0)
        end_time=tm.perf_counter()
        totalResults.append({'TimeConsumed':(end_time-start_time)/repeat})
