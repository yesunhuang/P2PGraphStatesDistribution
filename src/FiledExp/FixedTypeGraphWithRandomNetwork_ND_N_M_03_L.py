'''
Name: FixedTypeGraphWithRandomNetwork_ND_N_M_03_L
...
#%% data Path
EXP_NAME='FixedTypeGraphWithRandomNetwork_ND_N_M_03_L'
...
if __name__=='__main__':
    ...
    while True:
        P2PTask=p2pT.P2PTask(nodes,pc=pc,channels=channels,topology='Waxman',\
                        seed=globalSeed,Params=networkParams,ifFullControllable=True, memory=memory)
    ...
    print('Average channel probability:',np.mean([P2PTask.network_graph[u][v]['prob'] \
                                        for u,v in P2PTask.network_graph.edges()]))
    print('Average memory:',np.mean([P2PTask.network_graph.nodes[n]['memory'] \
                                        for n in P2PTask.network_graph.nodes()]))
...
if __name__=='__main__':
    ...
    vertexGroupNums=np.linspace(3,int(np.sqrt(5*nodes)),groupNum,dtype=int)
    vertexGroupNums=vertexGroupNums**2
    ...
    for i in range(0,groupNum):
        ...
        for j in range(0,batchNum):
            while True:
                task=p2pT.P2PTask(nodes,pc=pc,channels=channels, memory=memory,\
                            topology='Waxman',seed=int(graphSeeds[i,j]),Params=networkParams,\
                            ifFullControllable=True,Tracking=(i,j))
                task.pc=1
            ...
            graphStates=task.generate_distribution_task([vertexNum]*batchSize,topology='grid',\
                                                        memoryConstraint='flexible')\
            #modify the memory for MGST to execute
            chosenCenterNode=globalRng.integers(0,nodes)
            task.network_graph.nodes[chosenCenterNode]['memory']=2*int(np.max(vertexGroupNums))
            ...
            for k in range(0,batchSize):
                ...
                data[(i,j,k)]['AvgWidth']=np.mean([task.network_graph[u][v]['width']\
                                                    for u,v in task.network_graph.edges()])
                data[(i,j,k)]['AvgMemory']=np.mean([task.network_graph.nodes[n]['memory']\
                                                    for n in task.network_graph.nodes()])
