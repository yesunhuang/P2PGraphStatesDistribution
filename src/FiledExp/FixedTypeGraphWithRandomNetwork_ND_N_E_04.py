'''
Name: FixedTypeGraphWithRandomNetwork_ND_N_E_04
...
#%% data Path
EXP_NAME='FixedTypeGraphWithRandomNetwork_ND_N_E_04'
...
if __name__=='__main__':
    ...
    channels=1
    def pc(x):
        return globalRng.uniform(0.85,0.95)
    networkParams=2,0.3
    while True:
        P2PTask=p2pT.P2PTask(nodes,pc=pc,channels=0,topology='Cell',\
...
if __name__=='__main__':
    ...
    vertexGroupNums=np.linspace(4,int(nodes),groupNum,dtype=int)
    ...
    for i in range(0,groupNum):
        ...
        for j in range(0,batchNum):
            while True:
                task=p2pT.P2PTask(nodes,pc=pc,channels=channels,topology='Cell',\
                            seed=int(graphSeeds[i,j]),Params=networkParams,ifFullControllable=True,Tracking=(i,j))
                task.pc=1
                #reset the channel width
                for edge in task.network_graph.edges():
                    task.network_graph[edge[0]][edge[1]]['width']=channels
            ...
            graphStates=task.generate_distribution_task([vertexNum]*batchSize,Params=graphParams,topology='regular')
