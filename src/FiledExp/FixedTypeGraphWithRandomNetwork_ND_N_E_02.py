'''
Name: FixedTypeGraphWithRandomNetwork_ND_N_E_02
...
#%% data Path
EXP_NAME='FixedTypeGraphWithRandomNetwork_ND_N_E_02'
...
if __name__=='__main__':
    ...
    vertexGroupNums=np.linspace(20,4*nodes,groupNum,dtype=int)
    graphSeeds=globalRng.integers(0,100000,(groupNum,batchNum,batchSize))
    graphParams=0.05+0.1*globalRng.random((groupNum,batchNum,batchSize))

    ## save the seeds and params
    seedFileName='graphSeeds.npy'
    np.save(os.path.join(EXP_PATH,seedFileName),graphSeeds)
    paramFileName='graphParams.npy'
    np.save(os.path.join(EXP_PATH,paramFileName),graphParams)
    ...
    for i in range(0,groupNum):
        ...
        for j in range(0,batchNum):
            ...
            graphStates=task.generate_distribution_task([vertexNum]*batchSize,seed=graphSeeds[i,j,:],\
                                        topology='prufer_tree',Params=graphParams[i,j,:])
            ...
            for k in range(0,batchSize):
                ...
                data[(i,j,k)]['AvgDegree']=np.mean([np.mean([len(list(g.neighbors(v))) for v in g.nodes()]) for g in graphStates])
