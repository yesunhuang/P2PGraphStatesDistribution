'''
Name: FixedTypeGraphWithRandomNetwork_VW_N_E_03.py
...
#%% data Path
EXP_NAME='FixedTypeGraphWithRandomNetwork_VW_N_E_03'
...
#%% simulation methods
EXP_METHODS=['MGST','P2PGST','P2PGST-Max','ST-P2PGST-memShot']
...
if __name__=='__main__':
    ...
    vertexGroupNums=64
    channels=np.linspace(1,21,groupNum)
    ...
    for i in range(0,groupNum):
        ...
        for j in range(0,batchNum):
            while True:
                task=p2pT.P2PTask(nodes,pc=pc,channels=int(channels[i]),topology='Waxman',\
                            seed=int(graphSeeds[i,j]),Params=networkParams,\
            ...
            graphStates=task.generate_distribution_task([vertexGroupNums]*batchSize,topology='grid')
            ...
            for k in range(0,batchSize):
                ...
                data[(i,j,k)]['AvgDegree']=np.mean([len(list(task.network_graph.neighbors(n))) for n in task.network_graph.nodes()])
...
def run_single_batch_simulation(task:p2pT.P2PTask, method:str='MGST',**kargs):
    ...
    match method:
        ...
        case 'ST-P2PGST-memShot':
            ...
            options['MemoryStrategy']='Protective'
