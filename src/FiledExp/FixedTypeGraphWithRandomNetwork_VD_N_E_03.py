'''
Name: FixedTypeGraphWithRandomNetwork_VD_N_E_03.py
...
#%% data Path
EXP_NAME='FixedTypeGraphWithRandomNetwork_VD_N_E_03'
...
if __name__=='__main__':
    ...
    vertexGroupNums=64
    networkParams=(np.linspace(0.6,1.0,groupNum),np.linspace(0.2,1.0,groupNum))
        ...
        for j in range(0,batchNum):
            while True:
                task=p2pT.P2PTask(nodes,pc=pc,channels=channels,topology='Waxman',\
                            seed=int(graphSeeds[i,j]),Params=(networkParams[0][i],networkParams[1][i]),\
